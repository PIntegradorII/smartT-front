...
export default function RutinaPage({ datosHistoricos, mostrarDetalles = false }: RutinaProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const response = mostrarDetalles
        ? datosHistoricos
        : await getTrainingPlanByGoogleId(googleId);
  ...
  const generateRoutineHTML = () => {
    ...
    for (let i = 0; i < days.length; i += 2) {
      ...
      // Si queda una sola columna, rellenamos con un espacio vacío a la derecha
      if (isSingleColumn) {
        ...
        {Object.keys(dayMapping).map((day) => (
          <button
            ...
            className={`rounded-full px-6 py-2 text-sm font-medium transition-colors ${
              activeDay === day ? "bg-primary text-white" : "bg-white border border-gray-200 text-gray-700 hover:bg-gray-50"
            }`}
          >
            {day.charAt(0).toUpperCase() + day.slice(1)}
          </button>
