...
import { img_alimento } from "@/services/img_alimento/img_alimento";
...
const sendImage = async () => {
  ...
  try {
    const response = await img_alimento(selectedFile);
  ...
  return (
    <MainLayout>
      <div className="container mx-auto py-8 px-4">
        <h1 className="text-3xl font-bold text-center mb-8">Identificador de Alimentos</h1>

        <div className="max-w-3xl mx-auto bg-card rounded-lg shadow-lg p-6 flex flex-col items-center text-center">

          <div className="mb-6">
            <h2 className="text-xl font-semibold mb-4">Sube o toma una foto tus alimentos</h2>
            {errorMessage && (
          ...
          <Dialog open={showResults} onOpenChange={() => setShowResults(false)}>
            <DialogContent>
              ...
              <div className="w-full max-h-[70vh] overflow-y-auto pr-2">
                {foodData?.alimentos?.length > 0 ? (
                  foodData.alimentos.map((alimento: any, index: number) => (
                    <Card key={index} className="w-full shadow-md rounded-2xl mb-4">
                      <CardHeader>
                        <CardTitle className="text-xl font-semibold">
                          {alimento.nombre || "Nombre no disponible"}
                        </CardTitle>
                        <CardDescription className="text-sm text-muted-foreground">
                          {alimento.calorias ? `${alimento.calorias} Calorías` : "Calorías no disponibles"}
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-2">
                        <div>
                          <p className="font-semibold">Beneficios:</p>
                          <p>{alimento.beneficios || "No disponible"}</p>
                        </div>
                        <div>
                          <p className="font-semibold">Contraindicaciones:</p>
                          <p>{alimento.contraindicaciones || "No disponible"}</p>
                        </div>
                        <div>
                          <p className="font-semibold">Descripción:</p>
                          <p>{alimento.descripcion || "No disponible"}</p>
                        </div>
                      </CardContent>
                    </Card>
                  ))
                ) : (
                  <p className="text-center text-muted-foreground">No se encontraron alimentos.</p>
                )}
              </div>
